...
import { appendAssistantText } from "@/features/chat/store/helpers"
    ...
    try {
      ...
      for await (const event of parseSSE<WithMetadata<RuntimeYield>>(response.body)) {
        set((state) => {
          ...
          if (event.type === "agent-stream" && isRootAgent) {
            // append text to the assistant response
            if (event.part.type === "text-delta" && event.part.text) {
              appendAssistantText(target, event.part.text)
            }
      ...
      // if we encounter an unexpected error, update the assistant message with an error message (doesn't include errors that are part of the stream as events)
      set((state) => {
        const target = state.messages.find((message): message is UiAssistantMessage => message.id === assistantMessage.id)

        if (!target) return state

        target.parts = [{ type: "text", text: "Sorry, something went wrong. Please try again." }]
        target.streaming = false
