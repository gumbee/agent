...
import { StructuredJson, clean, type DynamicRegistry, type DeepPartial } from "@gumbee/structured"
...
export function createStructuredTransform<T>(options: StructuredTransformOptions<T> = {}): StructuredTransformResult<T> {
  ...
  const stream = new TransformStream<string, T>({
    ...
    flush() {
      ...
      // If parser finished but onComplete wasn't called, we need to resolve the promise
      // to prevent hanging. This can happen when:
      // 1. Incomplete JSON (parser.wasIncomplete = true) - resolve with partial value
      // 2. No JSON found at all (parser.value = undefined) - reject with error
      if (parser.value !== undefined) {
        // We have a value - resolve with it (whether complete or incomplete)
        resolveObject(clean(parser.value as T))
      } else {
        // No JSON was ever parsed - reject to prevent hanging
        rejectObject(new Error("No JSON content found in response"))
      }
    },
  })
